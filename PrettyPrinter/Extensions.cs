		...
		static string PrettyPrintThis(string value) {
			...
			foreach(var c in value) {
				switch(c) {
					...
					case '\\': ret += "\\\\"; break;
					case '"': ret += "\\\""; break;
					case char _ when char.IsControl(c) || c < ' ' || c >= 0x7f && c <= 0xff: ret += $"\\x{(int) c:x02}"; break;
					case char _ when c > 0xff: ret += "\\u..."; break; // TODO: Implement algo from unicodeescape_string: https://svn.python.org/projects/python/trunk/Objects/unicodeobject.c
					default: ret += c; break;
				}
			}
			return ret + "\"";
